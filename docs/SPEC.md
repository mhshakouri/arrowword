...
**The salvage that was meant to rescue broken replies looked in the wrong place.** It captured only objects at depth zero, and a layout arrives as `{"entries":[{...},{...}` where every entry is nested one level in. On a truncated reply the outer brace never closes, so the complete entries inside it were never even considered. The whole point of salvage is to rescue the intact parts of a broken document, and those parts are exactly the nested ones.

**The model has one repeatable malformation and it is worth repairing rather than rejecting.** From the second candidate onwards it writes `{"answer":"ORBIT","clue:"Path around a star"}`: the colon migrates inside the key's closing quote, the key becomes the string `clue:`, and the document is invalid from there. Twelve candidates, eleven lost, and the survivor was the one written before the model slipped into the pattern. The repair rewrites `"word:"` into `"word":` and nothing else, so it cannot turn a wrong answer into a plausible one; it fixes punctuation that makes valid content unreadable. **The tests for all three are the real replies, pasted verbatim**, because invented input tests a parser against what its author imagined a model does.

**The shared daily pool cannot be scoped by a test, and that broke the suite.** Per-IP keys carry a per-run prefix; the pool is global by definition and its counter lives in `.wrangler` between runs, so a day of test runs drained it and the suite began failing on "out of budget for today" rather than on anything it was checking. `GENERATION_POOL_KEY` is test-only and absent from `wrangler.jsonc`, alongside `RETENTION_MS` and the rest.
...
- **Tests could not see it.** The recorded provider resolves in a microtask and finishes before the handler returns, so every automated check passed. The one case that mattered is the one no fixture can reach, which is the same shape as the model-id bug and should have been the lesson then.
- **The early failures looked like model failures.** While the model id was wrong, `ai.run` threw instantly, so the loop completed inside the invocation and produced real messages about themes and word lists. Those messages were true; they just stopped being possible the moment the id was fixed.
- **A day was spent tuning things that never ran.** The model was switched, the prompts rewritten, the temperature lowered, JSON mode added. All of it may be an improvement and none of it was the fault. The model was switched back on the same reasoning: a change made to fix a defect it did not have is a decision resting on false evidence.

Generation now runs from an **alarm**, which is the documented way to do work in a Durable Object after a response has gone out: it is scheduled as its own invocation with its own lifetime and retried if it fails. The object has one alarm slot and two jobs, so the handler dispatches on a stored `pendingGeneration` and re-arms expiry afterwards, because an alarm spent on generation is an expiry alarm not set.

**The layout path never numbered its entries.** `readEntries` stamps `number: 0` and nothing replaced it: only the packer numbered, in a private copy. The first model layout that ever validated would have rendered a grid with 0 on every starting square and a clue list of nothing but zeros. It never surfaced because no layout ever survived validation in production, which is the kind of defect that waits for the day everything else starts working. One shared `numberEntries` now serves both paths, since two implementations is how one path ended up with none.

**The generation transcript is shown in the app**, on both the waiting screen and the failure screen: every prompt sent, every reply received, what parsed, and what the validator objected to. Until this existed, diagnosing a failure meant `wrangler tail` on the author's laptop while somebody else pressed the button, which is not a thing a visitor can do and was slow even for the person who wrote it. It is stored and replayed on connect for the same reason the pack request is: a broadcast is not delivery.

**The layout prompt was rewritten 2026-08-04 after a review**, and the review was right about where the failures come from. The rules were all present; what was missing was everything they imply. A model with 4B active parameters was being asked to derive index arithmetic in its head, and a disagreeing crossing is almost always that derivation going wrong rather than the rule being misunderstood. So the prompt now states the arithmetic outright, carries one worked crossing with its shared square named, caps the puzzle at 5 to 8 answers and says a sparse grid is correct, and phrases the adjacency rule as something to do rather than something to avoid.

Input costs 9,091 neurons per million against 27,273 for output, so 150 extra input tokens is about 1.4 neurons. **Prompt length is nearly free here and output is what we pay for**, which is the argument for spending words generously on instructions and never on inviting the model to think out loud.

The repair prompt was worse and had been overlooked entirely: it restated neither the grid size nor any rule, so the model was asked to fix coordinates without being told the bounds, and "fix only what is listed" forbade the cheapest valid repair, which is usually to drop the offending entry. Both corrected.

Three things came with it. `temperature` is 0.2, because crossings suffer far more at a high setting than clues gain. `response_format` with a JSON schema is attempted and **falls back to a plain call if the model rejects it**, since Cloudflare's documentation says an unsupported schema returns an error and an error would have been reported to the player as an outage. And proper nouns are now allowed in both prompts rather than banned in one: the same theme produced a different kind of puzzle depending on which path ran, and a puzzle about rivers with no river in it is not what anyone asked for.

**Changed 2026-08-04: the model is `@cf/google/gemma-4-26b-a4b-it`**, and it is now configurable with `GENERATION_MODEL` so comparing two is a deploy rather than a pull request. The first model, an 8B, could not reliably produce a valid layout. That is unsurprising rather than a defect in it: the layout prompt asks for a constraint-satisfaction solution in one shot, and the layout ADR said in advance that a failing layout means lowering density or changing model before concluding the model cannot do it. Gemma 4 is larger and, unusually, **very slightly cheaper per generation**, so the ceiling below does not move.
...
**Model costs, for whenever this is revisited.** Ours is dominated by output tokens, roughly 200 in and 500 out, so a model's output rate is what decides the ceiling. At that shape: Gemma 4 26B and Qwen3 30B and GPT-OSS 20B all land near 16 neurons, Mistral Small 24B near 32, GPT-OSS 120B near 40, Llama 3.3 70B near 108, and the reasoning models far past that, with `deepseek-r1-distill-qwen-32b` near 231 because it charges 443,756 neurons per million output tokens. **Reasoning models are the expensive ones here almost by definition**, since a long thinking trace is output we pay for and do not use.

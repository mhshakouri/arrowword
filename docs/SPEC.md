...
- Human, partly done 2026-08-03: **a clip recorded in Safari played in Chrome and back, locally.** That is the pair that would have caught the container problem, since Safari cannot play webm at all, so the decision to send raw PCM in a WAV wrapper rather than a `MediaRecorder` container is confirmed rather than merely argued. It proves the encoding path and nothing about the network
- Human, still outstanding: one clip from a phone in Iran arrives and is audible. Desktop Safari is not iOS Safari, which has its own audio session and backgrounding behavior, and localhost is not a network. That check is the whole point of the milestone and no local test substitutes for it
